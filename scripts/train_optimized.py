...
from src.data.dataset_optimized import OptimizedAudioImageDataset
from src.data.dataset_wrapper import DatasetWrapper  # Safe caching wrapper
from src.data.safe_augmentation import SafeAugmentation  # Conservative augmentation
...
class CLAP2DiffusionTrainer:
    ...
    def setup_class_weights(self):
        """Setup class weights for balanced training."""
        # Class weights based on balanced dataset distribution
        # sqrt(max_count / class_count) for stable training
        self.class_weights = {
            'ocean_waves': 1.93,  # sqrt(100/27)
            'dog_barking': 1.51,  # sqrt(100/44)
            'helicopter': 1.03,   # sqrt(100/94)
            'thunder': 1.00,      # sqrt(100/99)
            'fire': 1.00,
            'engine': 1.00,
            'rain': 1.00,
            'siren': 1.00
        ...
        for cls, weight in sorted(self.class_weights.items()):
            print(f"  {cls:15s}: weight={weight:.2f}")
    ...
    def collate_fn(self, batch):
        ...
        classes = [item.get('class', 'unknown') for item in batch]  # Extract class labels
        
        # Apply safe augmentation if training
        if hasattr(self, 'augmentation') and hasattr(self, 'current_stage'):
            for i in range(len(batch)):
                audio[i], images[i] = self.augmentation(audio[i], images[i])
        ...
        return {
            ...
            'class': classes  # Include class labels for weighted loss
        }
    ...
    def setup_datasets(self):
        ...
        # Training dataset with caching wrapper
        train_dataset_base = OptimizedAudioImageDataset(
            ...
            augment=False  # Disable built-in augmentation, use SafeAugmentation instead
        )
        self.train_dataset = DatasetWrapper(
            train_dataset_base,
            cache_enabled=True,
            cache_size=500  # Cache 10% of 5000 samples
        )
        
        # Validation dataset (no caching needed, smaller)
        self.val_dataset = OptimizedAudioImageDataset(
            data_root=self.config['data']['data_dir'],
            split="val",
            audio_sample_rate=self.config['data'].get('sample_rate', 48000),
            audio_duration=self.config['data'].get('audio_duration', 10.0),
        ...
        # Initialize safe augmentation for training
        self.augmentation = SafeAugmentation(enabled=True, flip_prob=0.5)
    ...
    def setup_stage(self, stage: int):
        ...
        if stage == 1:
            ...
            # Only train gate parameter (use 0.1 as proven in troubleshooting)
            self.optimizer = torch.optim.AdamW(
                [self.attention_adapter.gate],
                lr=self.config['stage3'].get('learning_rate', 0.1),  # Fixed: was 1e-3, should be 0.1
                weight_decay=0
            )
    ...
    def train_step(self, batch):
        ...
        # Get class labels for weighted loss (if available)
        class_labels = batch.get('class', None)
        ...
        # Apply attention adapter with gate (Stage 3)
        if self.current_stage == 3:
            # Use attention adapter to modulate audio tokens with gate
            # audio_tokens is already projected to 768 dim, use it as context
        ...
        if class_labels is not None and hasattr(self, 'class_weights'):
            # Apply class weights for balanced training
            loss = nn.functional.mse_loss(model_pred.float(), noise.float(), reduction="none")
            # Average over spatial dimensions
            loss = loss.mean(dim=[1, 2, 3])
            # Apply per-sample weights
            weights = torch.tensor([self.class_weights.get(label, 1.0) for label in class_labels], 
    ...
    def train(self):
        ...
        for stage in range(1, 4):
            ...
            for step in range(self.num_steps):
                # Stage 3 early stopping check
                if stage == 3 and step > 50:  # Check after warmup
                    gate_value = torch.tanh(self.attention_adapter.gate).item()
                    if 0.35 <= gate_value <= 0.45:
                        print(f"\nEarly stopping: Gate reached target range: {gate_value:.4f}")
                        stage_log.write(f"\nEarly stopped at step {step}, gate={gate_value:.4f}\n")
                        stage_log.flush()
                        break
                ...
                with self.accelerator.accumulate(accumulate_model):
                    ...
                    if self.accelerator.sync_gradients:
                        ...
                        if self.current_stage == 1:
                            ...
                            params_to_clip.extend(list(self.audio_adapter.parameters()))
                        else:  # stage 3
                ...
                # Add gate monitoring for Stage 3
                if stage == 3:
                    gate_value = torch.tanh(self.attention_adapter.gate).item()
                    logs["gate"] = gate_value
